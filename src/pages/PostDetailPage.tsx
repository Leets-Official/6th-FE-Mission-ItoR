...
/* ===== 섹션: 댓글 헤더 ===== */
const CommentsHeader: React.FC<{ count: number }> = ({ count }) => (
  <>
    ...
    <div className="flex max-w-[688px] px-4 items-center gap-2">
      <span className="text-[16px] leading-[25.6px] font-normal tracking-[-0.04px] text-[var(--Black)]">댓글</span>
      <span className="text-[16px] leading-[25.6px] font-medium tracking-[-0.04px] text-[var(--Point)]">{count}</span>
    </div>
  </>
);
...
/* ===== 섹션: 댓글 입력 ===== */
const CommentInput: React.FC<{ value: string; onChange: (v: string) => void; onSubmit: () => void }> = ({
  value,
  onChange,
  onSubmit,
}) => (
