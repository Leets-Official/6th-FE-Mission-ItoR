...
export const loginFormVariants = cva(
  'flex min-w-login-form-min-wide px-login-form-px py-0 flex-col items-center gap-login-form-gap flex-1'
);
...
export const snsDividerContainerVariants = cva(
  'flex w-sns-divider min-h-sns-divider justify-center items-center gap-0.5'
);
...
export const inputFieldVariants = cva(`${baseFieldStyle} placeholder:text-gray-56`);

// 버튼 필드 variants (button 전용)
export const buttonFieldVariants = cva(
  `${baseFieldStyle} cursor-pointer text-left text-gray-56 hover:text-gray-dark transition-colors`
);

// 로그인 버튼 variants
export const loginButtonVariants = cva(
  'flex h-login-button px-login-button-px justify-center items-center self-stretch w-full rounded-md font-medium text-sm cursor-pointer transition-colors',
  {
    variants: {
      variant: {
...
// SNS 버튼 텍스트 variants
export const snsButtonTextVariants = cva('font-semi-bold leading-[150%]', {
  variants: {
    provider: {
...
export const signupButtonVariants = cva('flex px-2 py-[2px] pb-1 justify-center items-center gap-1');

// 회원가입 텍스트 variants
export const signupTextVariants = cva('text-gray-56 text-sm font-light leading-[160%] tracking-[-0.07px]');
